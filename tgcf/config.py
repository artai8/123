...
class LoginConfig(BaseModel):  
    """Configuration for Telegram login with environment variable support."""  
    ...
    def __init__(self, **data):  
        # Auto-read from environment variables if not provided in data  
        if 'API_ID' not in data or data.get('API_ID') == 0:  
            api_id = os.getenv('TGCF_API_ID')  
            if api_id:  
                data['API_ID'] = int(api_id)  
          
        if 'API_HASH' not in data or not data.get('API_HASH'):  
            api_hash = os.getenv('TGCF_API_HASH')  
            if api_hash:  
                data['API_HASH'] = api_hash  
          
        if 'SESSION_STRING' not in data or not data.get('SESSION_STRING'):  
            session_string = os.getenv('TGCF_SESSION_STRING')  
            if session_string:  
                data['SESSION_STRING'] = session_string  
          
        if 'BOT_TOKEN' not in data or not data.get('BOT_TOKEN'):  
            bot_token = os.getenv('TGCF_BOT_TOKEN')  
            if bot_token:  
                data['BOT_TOKEN'] = bot_token  
          
        if 'user_type' not in data or data.get('user_type') == 0:  
            user_type = os.getenv('TGCF_USER_TYPE')  
            if user_type:  
                data['user_type'] = int(user_type)  
